...
const BUNDLE_BOILERPLATE: &str =
"--[[Generated with Jade (https://github.com/JaRoLoz)]]
...
impl DependencyNode {
    ...
    pub fn scan_dependencies(&self, base_path: &PathBuf, dependencies: &mut HashMap<String, DependencyNode>) {
...
impl<'a> Bundler<'a> {
    ...
    pub fn bundle(&mut self, base_path: &PathBuf) {
        let entrypoint = base_path.join(&self.config.entrypoint);
        let main_node = DependencyNode::new(entrypoint);
    ...
    pub fn write_bundle(&self, out_file: &mut File) {
        ...
        for (name, module) in &self.modules {
            let content = format!("____bundle__dict[\"{}\"] = function()\n{}\nend\n", name, module.get_contents());
            out_file.write_all(content.as_bytes()).unwrap();
        }
        out_file.write_all(&self.main_node.as_ref().unwrap().get_contents().as_bytes()).unwrap();
    }
}